...
def _call_llm_api(config: dict, messages: list, max_tokens: int = 4096) -> str:
    ...
    return _remove_think_tags(response.choices[0].message.content)


def _call_llm_api_with_retry(
    config: dict, messages: list, max_tokens: int = 4096
) -> str:
    return retry_call(
...
def generate_tldr(paper, config: dict) -> str:
...
def extract_keywords_from_paper(
    title: str, abstract: str, config: dict, max_keywords: int = 5
) -> list[str]:
...
def extract_keywords_from_corpus(
    papers_text: list[str], config: dict, max_keywords: int = 20
) -> list[str]:
    combined_text = "\n\n---\n\n".join(papers_text[:50])
